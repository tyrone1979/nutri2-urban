...
def apply_main(m):
    ...
    missing_section = (
        ...
        f"logit{{P(R=1 | FatER)}} = β₀ + β₁ z(FatER), where z(FatER) denotes FatER standardised on the "
        f"held-out test set (mean and SD computed on that test set only, matching the masking experiment), "
    ...
    s9_results = (
        ...
        f"{m['band_wide']['accuracy']:.3f} (Supplementary Table S9). This large swing, driven solely by a "
        "2-percentage-point shift in a threshold applied to a predictor variable, renders the three-class "
        "accuracy fundamentally non-interpretable as a measure of model performance and provides the decisive "
        "justification for focusing all primary conclusions on the binary Rural/Urban task "
